...
Matrix rowEchDet(Matrix m,
                 int *exchanges); // the same as rowEchelon() but it counts how
                                  // many exchanges were made (for detMatrix())

Boolean GJfindPivot(int start, Matrix c, int *pivot,
                    int *pivotR); // used in REF functions to find the pivot

void sub_matrixAdd(Matrix *m, mel el, int *row,
                   int *column); // adds into a matrix an element and moves
...
int compareRow(
    Row e1, Row e2,
    int dim); // for sorting, rows with smaller pivot indexes are "bigger"

void normalizeEl(Matrix *m, int pivotR, int j,
                 float norma); // normalize a sigle element

void op_gaussJordan(
    Matrix *c, Matrix *inverse); // performs reducedRowEch() the first, performs
                                 // the same operations on the second
...
int explicitVariable(L_EQ *eq); // for linear equations, makes the chosen
                                // variable (inside *eq) explicit
// e.g. id = 'a' , value =  a + 2b+ 3c , varIsolated = false -> returns id =
// 'a', value = -2b - 3c, varIsolated = true

int rowsToEquations(
    Matrix m,
    L_EQ *equations); // converts a matrix's rows into "unexplicited" equations

void rowsToEquationsEX(
    Matrix m,
    L_EQ *equations); // converts a matrix's rows into "explicited" equations

Boolean isInEquations_byID(
    int id, L_EQ *eqs,
    int dim); // checks if an equation of id "j" is present in the arrary "eqs"

Vect vectValue_byID(int id, L_EQ *eqs,
                    int dim); // finds the corresponding equation by id

void delRedundancyEq(L_EQ *equation, L_EQ *equations, int i,
                     int dim); // support function to make code look cleaner

void printL_EqEX(L_EQ eq); // print a linear equation (explicited)

void printL_EqsEX(L_EQ *eqs,
                  int rows); // same as above but an array of equations
...
Matrix emptyMatrix(int rows, int columns) {
  ...
  if (final.data == NULL || rows <= 0 || columns <= 0) {
    final = nullMatrix();
  } else {
    final.rows = rows;
    final.cols = columns;
...
Matrix inputMatrix() {
  ...
  while (i < empty.rows) {
    ...
    for (j = 0; j < empty.cols; j++) {
      scanned = 0;
      while (scanned < 1) {
        scanned = scanf("%f", &(empty.data[i][j]));
        if (scanned < 1) {
          clear_input();
...
int printMatrix(Matrix m) {
  ...
  if (m.data == NULL || m.rows == 0 || m.cols == 0)
    ...
    for (i = 0; i < m.rows; i++) {
      ...
      for (j = 0; j < m.cols; j++) {
        if (m.data[i][j] >= 10.0 ||
            (m.data[i][j] < 0.0 && m.data[i][j] > -10.0))
...
void freeVect(Vect v) { free(v.data); }
...
float laplaceDetMatrix(Matrix m) {
  ...
  if (!fullRank(m)) {
    ...
    if (m.rows == 1)
      ...
      while (i >= 0) {
        determinant += m.data[m.rows - 1][i] * (float)pow(-1, m.rows - 1 + i) *
                       laplaceDetMatrix(subMatrix(m, m.rows - 1, i));
        i--;
      }
    }
  }
...
int rankMatrix(Matrix m) {
  int rank;
  if (m.data == NULL || m.data == 0 || m.rows == 0 || m.cols == 0) {
    rank = 0;
  } else {
...
Matrix identityMatrix(int rows, int columns) {
  ...
  int i, j;
  if (identity.rows != identity.cols) {
...
Matrix inverseMatrix(Matrix m) {
  Matrix c, inverse;
  if (m.cols != m.rows || !fullRank(m)) {
    printf("\nInverse not allowed (this function only takes in full rank "
           "square matrices)\n");
    inverse = nullMatrix();
  } else {
    c = copyMatrix(m);
    ...
    // Reduces "c" to RREF while repeating the operations on "inverse" (identity
    // matrix)
...
void printKerMatrix(Matrix m) {
  int i;
  if (fullRank(m) || isZeroMatrix(m)) {
    printf("\nTrivial kernel\n");
  } else {
    Matrix RREF = reducedRowEch(m);
    delZeroRowsSorted(&RREF);
    if (fullRank(RREF) && RREF.rows == RREF.cols) {
      printf("\nTrivial kernel\n");
    } else {
      L_EQ *equations = NULL;
      ...
      if (equations == NULL) {
        ...
        for (i = 0; i < RREF.rows;
             i++) // DOUBLE check and free every vector correctly
          freeVect(equations[i].value);
        free(equations);
      }
      freeMatrix(RREF);
      // doesn't find free and null variables, i'll leave them to the user to
      // find as an exercise
    }
  }
}
...
void delRedundancyEq(L_EQ *equation, L_EQ *equations, int i, int dim) {
  ...
  for (j = equation->value.dim - 1; j > equation->id;
       j--) // for every entry of the equation
  {
...
int explicitVariable(L_EQ *eq) {
  int code;
  if (eq->id < 0 || isZeroRow(eq->value.data, eq->value.dim)) {
    code = 0;
  } else {
...
int rowsToEquations(Matrix m, L_EQ *equations) {
  int i = 0, j;
  if (m.data == NULL || m.rows == 0 || m.cols == 0 || equations->value.dim == 0)
    printf("\nInvalid inputs\n");
  else {
    for (i = 0; i < m.rows; i++) {
